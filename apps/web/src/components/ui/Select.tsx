...
const Select = React.forwardRef<HTMLButtonElement, SelectProps>(({
    ...
    const getSelectedDisplay = () => {
        ...
        if (multiple) {
            const valArray = value as (string | number)[];
            const selectedOptions = options?.filter(opt => Array.isArray(value) && valArray.includes(opt.value));
    ...
    const handleOptionSelect = (option: SelectOption) => {
        if (multiple) {
            const valArray = (Array.isArray(value) ? value : []) as (string | number)[];
            const updatedValue = valArray.includes(option.value)
                ? valArray.filter(v => v !== option.value)
                : [...valArray, option.value];
            onChange?.(updatedValue as string[] | number[]);
        } else {
    ...
    const isSelected = (optionValue: any) => {
        if (multiple) {
            const valArray = value as (string | number)[];
            return Array.isArray(value) && valArray.includes(optionValue);
        }
